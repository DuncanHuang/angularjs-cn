...
如果我们使用'Hi'文本输入框中的'Hello', 我们将看到如图1-3所示截图:
...
正如我们将在后面的章节中会看到, `$scope`并不是我们唯一可以访问的事情. 如果我们希望将数据绑定到用户浏览器的URL地址中, 我们可以通过将用于管理它的对象`$location`放在我们的构造器中来访问它, 就像这样:
...
我们通过Angular的依赖注入系统达到这个神奇的效果. 依赖注入让我们得以遵循开发的风格, 而不需要创建依赖, 我们的类只需要知道它需要什么.
